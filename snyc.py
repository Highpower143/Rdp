...
APPROVED_CHANNELS = [-1002204134287, -1002182851898, -100190128581, -1001819527882, -1002204134287]
APPROVED_GROUPS = [-1002182851898, -1001845623907]
def check_membership(user_id):
    # Check membership for all channels
    for chat_id in APPROVED_CHANNELS:
        try:
            status = bot.get_chat_member(chat_id, user_id).status
            if status not in ['member', 'administrator', 'creator']:
                return False  # If user is not a member of any channel, return False
        except Exception:
            return False  # In case of error (e.g., bot not being an admin), return False
    ...
    for chat_id in APPROVED_GROUPS:
        try:
            status = bot.get_chat_member(chat_id, user_id).status
            if status not in ['member', 'administrator', 'creator']:
                return False  # If user is not a member of any group, return False
        except Exception:
            return False  # In case of error, return False

    return True  # Return True only if the user is a member of all channels and groups
    def send_join_prompt(chat_id):
    markup = InlineKeyboardMarkup()
    for channel_url in ["https://t.me/creativeydv", "https://t.me/nandyadu1c", "https://t.me/v2ddosfeedback"]:
        join_button = InlineKeyboardButton(text="Join Channel", url=channel_url)
        markup.add(join_button)

    for group_url in ["https://t.me/v2ddos", "https://t.me/v2ddos_chatzone"]:
        join_button = InlineKeyboardButton(text="Join Group", url=group_url)
        markup.add(join_button)

    bot.send_message(chat_id, "You need to join these channels/groups to use the bot:", reply_markup=markup)
